...
class ServiceChatting extends StatefulWidget {
  ServiceChatting({Key key, this.data, this.receiver, this.flag, this.info})
      : super(key: key);
  ServiceChattingState createState() => ServiceChattingState();
  final List<Map> data;
...
class ServiceChattingState extends State<ServiceChatting> {
  ...
  void initState() {
    ...
    debugPrint("Token = ${widget.info}");
    debugPrint("*** Wiget info = ${widget.info}");
    http.post(Uri.parse(serverUrl + '/api/getuserinfo'), headers: {
      ...
      if (res.statusCode == 200) {
        ...
        DatabaseReference ref =
            FirebaseDatabase.instance.ref('$dbService/' + widget.receiver.toString());
  ...
  void sendMessage(message) async {
    DatabaseReference ref =
        FirebaseDatabase.instance.ref('$dbService/' + widget.receiver);
